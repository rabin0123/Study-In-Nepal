...
export default function StudyInNepalSection() {
  return (
    <section className="relative w-full bg-[#10161D] text-[#F3EFE6] overflow-hidden">
      <style>{`
      ...
      <svg
        className="absolute inset-x-0 bottom-0 w-full h-40 opacity-[0.08] pointer-events-none"
      ...
      <div className="relative max-w-6xl mx-auto px-6 py-20 md:py-28">
        {/* eyebrow */}
        <div className="sin-mono text-xs tracking-[0.25em] uppercase text-[#4FA79B] mb-5">
        ...
        <div className="max-w-2xl">
          <h2 className="sin-display text-4xl md:text-5xl leading-tight font-medium">
            ...
            <span className="text-[#E7A94C]"> one simple application.</span>
          </h2>
          <p className="sin-body text-[#8CA0AC] mt-5 text-base md:text-lg leading-relaxed">
        ...
        <div className="mt-16 mb-16">
          <div className="relative flex items-start justify-between max-w-3xl">
            <div className="absolute top-5 left-0 right-0 h-px bg-[#2A3742]" />
            {journey.map((step, i) => {
              const Icon = step.icon;
              return (
                <div key={step.label} className="relative flex flex-col items-center text-center w-1/4 px-2">
                  <div className="w-10 h-10 rounded-full bg-[#1B242E] border border-[#2A3742] flex items-center justify-center z-10">
                    <Icon size={16} className="text-[#4FA79B]" />
                  </div>
                  <div className="sin-mono text-[10px] tracking-widest uppercase text-[#8CA0AC] mt-3">
                    0{i + 1}
                  </div>
                  <div className="sin-body text-sm font-medium mt-1">{step.label}</div>
                  <div className="sin-body text-xs text-[#8CA0AC] mt-1 hidden md:block">
                    {step.detail}
                  </div>
                </div>
              );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {universities.map((u) => (
            <div
              key={u.name}
              className="bg-[#1B242E] border border-[#2A3742] rounded-lg p-6 flex flex-col justify-between hover:border-[#4FA79B] transition-colors"
            >
              <div>
                <div className="flex items-center gap-1.5 sin-mono text-[11px] uppercase tracking-wider text-[#8CA0AC]">
                  <MapPin size={12} />
                  {u.location}
                </div>
                <h3 className="sin-display text-xl font-medium mt-3">{u.name}</h3>
                <p className="sin-body text-sm text-[#C9D2D8] mt-2">{u.course}</p>

                <div className="flex items-center gap-4 mt-4 pt-4 border-t border-[#2A3742]">
                  <div className="flex items-center gap-1.5 sin-mono text-xs text-[#8CA0AC]">
                    <Clock size={12} />
                    {u.duration}
                  </div>
                  <div className="flex items-center gap-1.5 sin-mono text-xs text-[#E7A94C]">
                    <Wallet size={12} />
                    {u.fee}
                  </div>
                </div>
              ...
              <button className="sin-body group flex items-center justify-between text-sm mt-6 text-[#F3EFE6] hover:text-[#4FA79B] transition-colors">
                View program
        ...
        {/* CTA */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mt-16 pt-10 border-t border-[#2A3742]">
          <p className="sin-body text-sm text-[#8CA0AC] max-w-sm">
            Sample data shown above — connect your own university and course
            list to populate this section.
          </p>
          <button className="sin-body bg-[#E7A94C] text-[#10161D] font-medium text-sm px-6 py-3 rounded-md hover:bg-[#F3BE6C] transition-colors whitespace-nowrap">
