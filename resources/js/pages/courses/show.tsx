...
export default function CourseDetailsShow({ courseDetail }: Props) {
    ...
    const isLinked = Boolean(courseDetail.university_id);
        ...
        >
            ...
            <div className="card mb-4">
                <div className="card-body d-flex align-items-center gap-3 flex-wrap">
                    ...
                    <div className="flex-grow-1">
                        <h3 className="mb-1 fw-semibold">{courseDetail.course_name}</h3>
                        <p className="mb-0 text-body-secondary">
                            {courseDetail.college_name} · {courseDetail.university_name}
                        </p>
                        <div className="d-flex flex-wrap gap-2 mt-2">
                            {courseDetail.university?.level && (
                                <span className="badge text-bg-secondary-subtle text-secondary">{courseDetail.university.level}</span>
                            )}
                            {courseDetail.university?.Intake && (
                                <span className="badge text-bg-info-subtle text-info">Intake: {courseDetail.university.Intake}</span>
                            )}
                            {courseDetail.university?.Location && (
                                <span className="badge text-bg-light text-body-secondary">{courseDetail.university.Location}</span>
                            )}
                            {!isLinked && (
                                <span className="badge text-bg-warning-subtle text-warning" title="No matching university/college/course row found yet">
                                    Not yet linked to a university listing
                                </span>
                            )}
                        </div>
                    </div>
                    <Link href={`/course-details/${courseDetail.uuid}/edit`} className="btn btn-outline-primary">
                        Edit
                    </Link>
                </div>
            </div>
            ...
            <div className="row">
                    ...
                    {!courseDetail.summary && modules.length === 0 && careers.length === 0 && (
                        <div className="card">
                            <div className="card-body text-center py-10">
                                <p className="text-body-secondary mb-3">No summary, modules, or careers added yet.</p>
                                <Link href={`/course-details/${courseDetail.uuid}/edit`} className="btn btn-primary">
                                    Add details
                                </Link>
                            </div>
                        </div>
                ...
                <div className="col-lg-4">
                                            ...
                                            {fees.map((fee) => (
                                                <tr key={fee.year} className="border-top">
                                                    ...
                                                    <td className="py-2 fw-semibold">
                                                        ...
                                                        {fee.note && <div className="fs-2 text-body-secondary fw-normal">{fee.note}</div>}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                    ...
                    <div className="card">
                        <div className="card-body">
                            <Link href={`/course-details/${courseDetail.uuid}/edit`} className="btn btn-outline-primary w-100">
                                Edit course details
                            </Link>
                        </div>
                    </div>
